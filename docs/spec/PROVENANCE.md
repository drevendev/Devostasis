...
| exact vector families T2/R1/R2, R3, R4, T4/T5/T7 | PV-TEST-VECTORS-002/005/004/007, accepted by PV-REV-TEST-VECTORS-002/005/004/007 | transcribed into `tests/vectors/` in 0.1.9 as the accepted expectations; T5 (`PULSE_ISSUE_ONLY_ACTIVITY`) and T7 (`retention_semantics = UNCLASSIFIED` upper bound) adopted in the evaluators under their existing rule ids, as the accepted vectors require |
| `devostasis.vectors.v1` | implementation choice ahead of PV-TEST-001 | local: the format and runner are this repository's, so the vectors of PV-TEST-001 arrive executable rather than needing translation; 0.1.9 adds the `ci` and `activity` kinds and `variants` in answer to the format findings of issues #20 and #23 |
